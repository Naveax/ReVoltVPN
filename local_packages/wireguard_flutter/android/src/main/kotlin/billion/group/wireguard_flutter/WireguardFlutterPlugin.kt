...
class WireguardFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    private fun createBackend(): Backend {
        if (backend == null) {
            backend = GoBackend(context, object : TunnelActionHandler {
                override fun runPreUp(commands: Collection<String>) {}
                override fun runPostUp(commands: Collection<String>) {}
                override fun runPreDown(commands: Collection<String>) {}
                override fun runPostDown(commands: Collection<String>) {}
            })
        }
    ...
    private fun disconnect(result: Result) {
        scope.launch(Dispatchers.IO) {
            try {
                if (futureBackend.await().getRunningTunnelNames().isEmpty()) {
