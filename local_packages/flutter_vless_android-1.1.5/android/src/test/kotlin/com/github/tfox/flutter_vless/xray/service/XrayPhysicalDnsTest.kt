...
class XrayPhysicalDnsTest {
    private fun query(type: Int) = byteArrayOf(
        0x12,
        0x34,
        1,
        0,
    ...
    @Test
    fun oldAndroidAddressBridgePreservesQuestionAndFamily() {
        for (type in listOf(1, 28)) {
            ...
            assertArrayEquals(
                query(type).copyOfRange(12, 24),
                response.copyOfRange(12, 24),
            )
            assertEquals(if (type == 1) 40 else 52, response.size)
        }

        val unsupported = XrayPhysicalDns.addressQuery(query(16)) {
            error("Unsupported queries must not escape through another resolver")
        }
