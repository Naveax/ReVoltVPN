...
/**
 * Owns the in-process Xray runtime used by ReVolt.
 *
 * Security invariants:
...
object XrayCoreManager {
    ...
    internal fun buildRuntimeConfigJson(config: XrayConfig, filesDir: File): JSONObject {
        ...
        val inbounds = configJson.optJSONArray("inbounds")
            ?: throw IllegalStateException("Authenticated ReVolt SOCKS5 inbound is missing")
        if (inbounds.length() != 1) {
            throw IllegalStateException("ReVolt runtime must expose exactly one local inbound")
        }

        val inbound = inbounds.optJSONObject(0)
            ?: throw IllegalStateException("Authenticated ReVolt SOCKS5 inbound is malformed")
        ...
        if (users.length() != 1) {
            throw IllegalStateException("Secure SOCKS5 must contain exactly one account")
        }
    ...
    fun startCore(context: XrayVPNService, config: XrayConfig): Boolean {
        ...
        try {
            val pb = ProcessBuilder(xrayExecutable.absolutePath, "-config=stdin:")
    ...
    fun sendStatusBroadcast(context: Context, config: XrayConfig, queryId: String? = null) {
        ...
        queryId?.let { intent.putExtra("QUERY_ID", it) }
        context.sendBroadcast(intent)
    }

    fun sendNoRuntimeBroadcast(context: Context, queryId: String) {
        val intent = Intent(AppConfigs.V2RAY_CONNECTION_INFO).setPackage(context.packageName)
        intent.putExtra("STATE", AppConfigs.V2RAY_STATES.V2RAY_DISCONNECTED)
        intent.putExtra("RUNTIME_TOKEN", "")
